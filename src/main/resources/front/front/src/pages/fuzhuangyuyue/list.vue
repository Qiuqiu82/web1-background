    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.yonghuxingming != '') searchWhere.yonghuxingming = '%' + this.formSearch.yonghuxingming + '%';
	      params['sfsh'] = '�?;
        this.$http.get('fuzhuangyuyue/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/fuzhuangyuyueDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
