...
export default {
  ...
  methods:{
    ...
    getTimeStrToDay(game_over_timestamp) {
        ...
        let arr = ["æ—?, "ä¸€", "äº?, "ä¸?, "å›?, "äº?, "å…?];
        let d = arr[date.getDay()]
        return date.getFullYear() + "å¹? + (date.getMonth() + 1) + "æœ? + date.getDate() + 'æ—? + ' ' + ' ' + 'æ˜ŸæœŸ' + d  + ' ' + "  " + hours + ":" + minutes + ":" + seconds
    },
	times() {
		setInterval(()=>{
	...
	filterTime(time) {
	  ...
	  const H = date.getHours() < 10 ? '0' + date.getHours() : date.getHours() // å°æ—¶
	  const I = date.getMinutes() < 10 ? '0' + date.getMinutes() : date.getMinutes() // åˆ†é’Ÿ
